...
describe Api::RigsController do
  ...
  before do
    stub_const('Rig::Store', double('Rig::Store', build: store))
  end
  ...
  describe "GET #index" do
    ...
    before do
      allow(store).to receive(:find_all){ rigs }
  ...
  describe "GET #show" do
    context "for a rig that exists" do
      ...
      before do
        allow(store).to receive(:find_by_id){ rig }
  ...
  describe "POST #create" do
    ...
    context "for a valid rig" do
      ...
      it { expect(store).to have_received(:create).with(rig_form) }
    end
